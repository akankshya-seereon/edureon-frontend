...
export default function InstituteAttendance() {
  ...
  const handlePunchOutAll = () => {
    ...
    facultyList.forEach((faculty) => {
      ...
      if (rec?.punchIn && !rec.punchOut && (rec.status === "Present" || rec.status === "Late")) {
    ...
    <div className="min-h-screen bg-slate-50" style={{ fontFamily: "'Georgia', serif" }}>
      <div className="w-full max-w-8xl mx-auto px-4 py-8 pb-32">
        ...
        <div className="mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-3" style={{ fontFamily: "sans-serif" }}>
            <div className="flex items-center gap-2 bg-white border border-slate-200 shadow-sm px-4 py-2.5 rounded-xl text-sm">
              ...
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="outline-none bg-transparent text-slate-700 font-semibold w-full" />
            </div>
            <div className="flex items-center gap-2 bg-white border border-slate-200 shadow-sm px-4 py-2.5 rounded-xl text-sm">
              <BookOpen size={14} className="text-slate-400 shrink-0" />
              <select value={selectedDept} onChange={(e) => setSelectedDept(e.target.value)} className="outline-none bg-transparent text-slate-700 font-semibold w-full">
          ...
          <div className="p-6" style={{ fontFamily: "sans-serif" }}>
            ...
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 w-full">
              ...
              <div className="flex flex-wrap items-center gap-3 flex-1 lg:justify-center">
                ...
                <div className="text-xs font-semibold text-slate-400 w-12 text-center shrink-0">
              ...
              {/* 3. Right Col: Actions */}
              <div className="flex flex-wrap items-center gap-3 shrink-0 lg:justify-end w-full lg:w-auto">
                <button onClick={handleAdminPunchIn} disabled={!shiftOn || !!adminRecord.punchIn} className={`flex items-center justify-center flex-1 lg:flex-none gap-2 px-5 py-2.5 rounded-xl text-sm font-bold border transition active:scale-95 shadow-sm whitespace-nowrap ${adminRecord.punchIn ? "bg-emerald-100 text-emerald-700 border-emerald-200 cursor-default" : shiftOn ? "bg-blue-600 text-white border-blue-600 hover:bg-blue-700 cursor-pointer" : "bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed"}`}>
                  <LogIn size={14} /> {adminRecord.punchIn ? "Punched In ✓" : "Punch In"}
                </button>
                <button onClick={handleAdminPunchOut} disabled={!shiftOn || !adminRecord.punchIn || !!adminRecord.punchOut} className={`flex items-center justify-center flex-1 lg:flex-none gap-2 px-5 py-2.5 rounded-xl text-sm font-bold border transition active:scale-95 shadow-sm whitespace-nowrap ${adminRecord.punchOut ? "bg-rose-100 text-rose-600 border-rose-200 cursor-default" : adminRecord.punchIn && shiftOn ? "bg-white text-slate-700 border-slate-300 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-300 cursor-pointer" : "bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed"}`}>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm mb-6 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3" style={{ fontFamily: "sans-serif" }}>
          ...
          <div className="p-6" style={{ fontFamily: "sans-serif" }}>
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={approveAll} disabled={pendingCount === 0} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 rounded-lg text-sm font-bold text-slate-700 hover:bg-emerald-50 hover:border-emerald-300 hover:text-emerald-700 transition disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap">
                <CheckCircle size={14} /> Approve All Pending
              </button>
              <button onClick={handlePunchOutAll} disabled={!shiftOn} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 rounded-lg text-sm font-bold text-slate-700 hover:bg-red-50 hover:border-red-300 hover:text-red-600 transition disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="px-5 pt-4 pb-0 bg-slate-50 border-b border-slate-200 overflow-x-auto no-scrollbar" style={{ fontFamily: "sans-serif" }}>
            <div className="flex gap-1 min-w-max">
              {TABS.map((tab) => (
                <button key={tab.id} onClick={() => setFilterTab(tab.id)} className={`px-4 py-2 text-xs font-bold rounded-t-lg border-b-2 transition-all whitespace-nowrap ${filterTab === tab.id ? "border-indigo-600 text-indigo-700 bg-white" : "border-transparent text-slate-500 hover:text-slate-700"}`}>
          ...
          <div className="divide-y divide-slate-100">
            {visibleFaculty.length === 0 && (
              <div className="p-8 text-center text-slate-500 text-sm font-semibold" style={{ fontFamily: "sans-serif" }}>
                No faculty records found for this filter.
              </div>
            )}
                ...
                <div key={faculty.id} className="p-4 hover:bg-slate-50 transition-colors">
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 w-full">
                    ...
                    <div className="flex flex-wrap items-center gap-2 lg:gap-3 flex-1 lg:justify-center" style={{ fontFamily: "sans-serif" }}>
                      ...
                      <div className="text-center text-xs font-semibold text-slate-400 w-10 shrink-0">
                      ...
                      <span className={`px-2.5 py-1 mt-1 lg:mt-0 rounded-lg text-[9px] font-black uppercase tracking-wider border whitespace-nowrap ${badge.cls}`}>
                        {badge.label}
                      </span>
                    </div>

                    {/* 3. Right Col: Actions */}
                    <div className="flex flex-wrap items-center justify-start lg:justify-end gap-2 w-full lg:w-auto shrink-0 mt-1 lg:mt-0" style={{ fontFamily: "sans-serif" }}>
                      {rec.status === "Pending" && (
                        <>
                          <button onClick={() => handleApprove(faculty.id)} className="flex-1 lg:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border bg-emerald-600 text-white border-emerald-700 hover:bg-emerald-700 transition active:scale-95 shadow-sm whitespace-nowrap">
                            <CheckCircle size={12} /> Approve
                          </button>
                          <button onClick={() => handleReject(faculty.id)} className="flex-1 lg:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border bg-red-50 text-red-600 border-red-300 hover:bg-red-100 transition active:scale-95 whitespace-nowrap">
                            <XCircle size={12} /> Reject
                          </button>
                        </>
                      )}
                      {(rec.status === "Present" || rec.status === "Late") && (
                        <button onClick={() => handlePunchOut(faculty.id)} disabled={!shiftOn || rec.punchOut} className={`flex-1 lg:flex-none flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border transition active:scale-95 whitespace-nowrap ${rec.punchOut ? "bg-rose-100 text-rose-600 border-rose-200" : shiftOn ? "bg-white text-slate-600 border-slate-300 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-300" : "bg-slate-50 text-slate-300 border-slate-200 cursor-not-allowed"}`}>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/95 backdrop-blur border-t border-slate-200 shadow-[0_-4px_20px_rgba(0,0,0,0.07)] flex flex-wrap gap-3 justify-between items-center z-10" style={{ fontFamily: "sans-serif" }}>
        ...
        <button onClick={handleSave} className={`w-full sm:w-auto px-6 py-2.5 rounded-xl font-bold text-sm flex justify-center items-center gap-2 transition shadow-lg ${saved ? "bg-emerald-600 text-white shadow-emerald-500/30" : "bg-blue-600 text-white hover:bg-blue-700 shadow-indigo-500/30"}`}>
