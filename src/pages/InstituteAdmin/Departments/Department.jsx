...
export const Department = () => {
  ...
  // ── 🚀 Fetch all initial data using centralized API ──
  const fetchInitialData = async () => {
    try {
      setLoading(true);

      const [deptsRes, empRes, infraRes] = await Promise.allSettled([
        api.get('/admin/departments'),
        api.get('/admin/employees/list'),
        api.get('/admin/departments/infrastructure'), // 🚀 FIXED: Using the properly scoped endpoint!
      ]);
      ...
      // Buildings & Rooms (Real Data from the new controller)
      if (infraRes.status === 'fulfilled' && infraRes.value.data?.success) {
        const data = infraRes.value.data.data;
  ...
  // ── 🚀 SMART CASCADING LOGIC FOR REAL ROOMS ──
  ...
  // Dynamically filter options based on what exists in the database
  const availableBlocks = useMemo(() => {
    if (!assignForm.building) return [];
    return [...new Set(rooms.filter(r => r.building === assignForm.building).map(r => r.block).filter(Boolean))];
  }, [rooms, assignForm.building]);

  const availableFloors = useMemo(() => {
    if (!assignForm.building) return [];
    return [...new Set(rooms.filter(r => 
      r.building === assignForm.building && 
      (!assignForm.block || r.block === assignForm.block)
    ).map(r => r.floor).filter(Boolean))];
  }, [rooms, assignForm.building, assignForm.block]);
  ...
  const availableRooms = useMemo(() => {
    ...
    return rooms.filter(r => 
      r.building === assignForm.building && 
      (!assignForm.block || r.block === assignForm.block) && 
      (!assignForm.floor || r.floor === assignForm.floor)
    );
  }, [rooms, assignForm.building, assignForm.block, assignForm.floor]);
  ...
  const handleAssignRoom = async (e) => {
    ...
    if (!assignForm.room.trim())  return showToast('error', 'Please select a room.');
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
          <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3 bg-slate-50/50 rounded-t-2xl">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 font-medium">Allocate specific real building rooms to a department.</p>
            </div>
          </div>
            ...
            <form onSubmit={handleAssignRoom} className="grid grid-cols-1 md:grid-cols-7 gap-4 items-end">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => handleVenueChange('departmentId', e.target.value)}
                  className="form-input"
                  style={{ height: '38px' }}
                  required
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => handleVenueChange('building', e.target.value)}
                  className="form-input"
                  style={{ height: '38px' }}
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => handleVenueChange('block', e.target.value)}
                  className="form-input disabled:bg-slate-100 disabled:text-slate-400"
                  style={{ height: '38px' }}
                  disabled={!assignForm.building || availableBlocks.length === 0}
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => handleVenueChange('floor', e.target.value)}
                  className="form-input disabled:bg-slate-100 disabled:text-slate-400"
                  style={{ height: '38px' }}
                  disabled={!assignForm.building || availableFloors.length === 0}
                >
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => handleVenueChange('room', e.target.value)}
                  className="form-input disabled:bg-slate-100 disabled:text-slate-400"
                  style={{ height: '38px' }}
                  disabled={!assignForm.building || availableRooms.length === 0}
                  required
                >
              ...
              <div style={{ height: '38px' }}>
                <button
                  type="submit"
                  disabled={isSubmitting || !assignForm.room}
                  className="w-full h-full flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white text-[11px] font-black rounded-lg transition-all shadow-md"
                >
