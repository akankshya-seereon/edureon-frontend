...
// ─── SEARCHABLE HOD DROPDOWN (Now explicitly handles Optional state) ──────────
    ...
    <div ref={ref} className="relative w-full overflow-visible">
      ...
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          {label ? (
            <>
              <div className="w-5 h-5 rounded-full bg-blue-600 flex items-center justify-center text-[9px] font-black text-white flex-shrink-0 uppercase">
        ...
        <div className="absolute z-[9999] mt-1 w-64 bg-white border border-gray-200 rounded-xl shadow-2xl overflow-hidden left-0">
          <div className="p-2 border-b border-gray-100 bg-gray-50">
            <div className="relative">
              ...
              <input autoFocus type="text" placeholder="Search staff..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div className="overflow-y-auto max-h-[220px]">
            {/* 🚀 Option to Keep/Set HOD as Null */}
...
export const Department = () => {
  ...
  const [formData, setFormData] = useState({
    name: '', department_code: '', hodId: '', category: '', type: 'Academic', roomNumber: '',
  });
  ...
  const handleAddDepartment = async (e) => {
    ...
    try {
      ...
      // 🚀 Explicitly handle HOD as Optional (send null if empty string)
      const payload = { ...formData, hodId: formData.hodId || null, type: activeTab };
      const res = await api.post('/admin/departments', payload);
      if (res.data.success) {
        await fetchInitialData();
        setFormData({ name: '', department_code: '', hodId: '', category: '', type: activeTab, roomNumber: '' });
      }
    } catch (err) {
  ...
  const handleUpdateDepartment = async (id) => {
    try {
      setIsSubmitting(true);
      // 🚀 Handle HOD update as optional
      const payload = { ...editFormData, hodId: editFormData.hodId || null };
  ...
  const filteredDepartments = useMemo(() => {
    return (Array.isArray(departments) ? departments : []).filter(d =>
      d.type === activeTab && (d.name || '').toLowerCase().includes(listSearch.toLowerCase())
    );
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-left overflow-visible">
      <style>{`
        .form-input { width: 100%; padding: .575rem .875rem; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: .5rem; font-size: .8125rem; font-weight: 600; color: #1e293b; outline: none; transition: all .15s; }
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center px-6 py-4 border-b border-gray-100 gap-4">
            ...
            <div className="flex gap-1 bg-gray-100 p-1 rounded-lg">
              {['Academic', 'Non-Academic'].map(t => (
                <button key={t} onClick={() => { setActiveTab(t); setEditingId(null); }}
          ...
          <div className="p-6 overflow-visible">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="relative w-56">
                ...
                <input type="text" placeholder="Filter list…" value={listSearch} onChange={e => setListSearch(e.target.value)}
                  className="w-full pl-8 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-xs font-semibold" />
              </div>
            </div>

            {loading ? (
              <div className="flex items-center justify-center py-16 gap-3 text-gray-400 font-bold italic"><Loader2 className="animate-spin text-blue-500" size={20} /> Loading...</div>
            ) : (
                      ...
                      {editingId === dept.id ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 px-5 py-4 items-center bg-blue-50/40 overflow-visible">
                          ...
                          <div className="md:col-span-1 flex items-center justify-end gap-1">
                            <button onClick={() => handleUpdateDepartment(dept.id)} className="p-1.5 text-green-600 hover:bg-green-100 rounded transition">
                              {isSubmitting ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />}
                            </button>
                            <button onClick={() => setEditingId(null)} className="p-1.5 text-gray-400 hover:bg-gray-200 rounded transition"><XCircle size={16} /></button>
                          </div>
                        </div>
                      ) : (
                        /* 👁️ VIEW STATE ROW */
                        <div className="grid grid-cols-12 gap-4 px-5 py-4 items-center hover:bg-slate-50/50 transition-colors group">
                          ...
                          <div className="col-span-3">
                            ...
                            ) : <span className="text-gray-300 text-xs italic font-medium">Unassigned (Optional)</span>}
                          </div>
                          <div className="col-span-2">
                            <span className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-black rounded uppercase tracking-wider">{dept.category || 'General'}</span>
                          </div>
                          <div className="col-span-2 text-xs font-bold text-slate-500">{dept.roomNumber || '—'}</div>
                          <div className="col-span-1 flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => handleEditClick(dept)} className="p-2 text-gray-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg"><Edit size={14} /></button>
                            <button onClick={() => handleDelete(dept.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  <form onSubmit={handleAddDepartment} className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end overflow-visible">
                    <div className="md:col-span-3">
                      ...
                      <input type="text" placeholder="e.g. Computer Science" value={formData.name}
                        onChange={e => setFormData(f => ({ ...f, name: e.target.value }))} className="form-input" required />
                    </div>
                    <div className="md:col-span-2">
                      ...
                      <input type="text" placeholder="CS-101" value={formData.department_code}
                        onChange={e => setFormData(f => ({ ...f, department_code: e.target.value }))} className="form-input" />
                    </div>
                    <div className="md:col-span-3 overflow-visible">
                      <label className="block text-[10px] font-black text-slate-400 mb-1 uppercase">Assign HOD (Optional)</label>
                      <HodDropdown faculty={eligibleFaculty} value={formData.hodId} onChange={v => setFormData(f => ({ ...f, hodId: v }))} />
                    </div>
                    <div className="md:col-span-2">
                      <label className="block text-[10px] font-black text-slate-400 mb-1 uppercase">Category</label>
                      <select value={formData.category} onChange={e => setFormData(f => ({ ...f, category: e.target.value }))} className="form-input h-[38px]" required>
                        <option value="">Select...</option>
                        {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                      </select>
                    </div>
                    <div className="md:col-span-1">
                      ...
                      <input type="text" placeholder="302" value={formData.roomNumber}
                        onChange={e => setFormData(f => ({ ...f, roomNumber: e.target.value }))} className="form-input" />
                    </div>
                    <div className="md:col-span-1 h-[38px]">
                      <button type="submit" disabled={isSubmitting} className="w-full h-full flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white text-[11px] font-black rounded-lg transition-all shadow-md">
                        {isSubmitting ? <Loader2 size={13} className="animate-spin" /> : <Plus size={13} />} ADD
                      </button>
                    </div>
                  </form>
                </div>
