...
import { 
  ...
  Edit2, Loader2, Package, X, AlertCircle, Users
} from 'lucide-react';
...
// ─── HELPER: resolve the final room type value ────────────────────────────────
// If user picked "Other" and typed a custom name, use that; else use the selected type.
...
const ToggleSwitch = ({ active, onToggle }) => (
  <div
    onClick={onToggle}
    className={`w-10 h-[22px] rounded-full flex items-center px-0.5 cursor-pointer transition-colors outline-none shadow-inner ${active ? 'bg-emerald-500' : 'bg-slate-300'}`}
  >
...
/**
 * RoomTypeField
 * Shows a <select> for standard types.
 * When "Other" is selected, shows an additional text input for the custom type name.
 */
const RoomTypeField = ({ value, customValue, onTypeChange, onCustomChange, required }) => (
  <div className="flex flex-col gap-1.5 w-full text-left">
    <label className="text-xs font-bold text-slate-600">
      Type{required && <span className="text-red-500 ml-0.5">*</span>}
    </label>
    <select
    ...
    {value === 'Other' && (
      <input
        type="text"
        placeholder="Specify room type (e.g. Storage, Workshop...)"
        value={customValue}
...
const EMPTY_ROOM = {
  ...
  type: '', customType: '',
  capacity: '', floor: '', block: '', building_id: ''
};
...
const InfrastructurePage = () => {
  ...
  const handleDeleteCampus = async (id) => {
    ...
    try { await infrastructureService.deleteCampus(id); loadData(); }
  ...
  const handleCreateBuilding = async (campusId) => {
    ...
    const isDuplicate = campus?.buildings.some(b => b.name.toLowerCase() === bData.name.trim().toLowerCase());
  ...
  const handleDeleteBuilding = async (id) => {
    ...
    try { await infrastructureService.deleteBuilding(id); loadData(); }
  ...
  /**
   * Central handler for type change.
   * Clears customType whenever a non-"Other" type is chosen.
   */
  ...
  const handleSaveRoom = async () => {
    ...
    // Resolve final type before validation/submission
    const finalType = resolveFinalType(newRoom.type, newRoom.customType || '');

    // Validate "Other" custom type
    if (newRoom.type === 'Other' && !newRoom.customType?.trim()) {
      return setFormError("Please specify the room type when selecting 'Other'.");
    }
    ...
    if (isBulkMode) {
      ...
      setSavingBulk(true);
      try {
        const promises = generatedRoomNumbers.map(roomNo =>
          infrastructureService.createRoom({
            roomNo,
            type: finalType,          // ← resolved custom type sent to API
  ...
  const handleEditRoom = (room) => {
    ...
    // If the stored type is not in the predefined list, treat as "Other" + custom
  ...
  const handleDeleteRoom = async (id) => {
    ...
    try { await infrastructureService.deleteRoom(id); loadData(); }
    catch (error) { console.error("Error deleting room", error); }
  };

  const handleToggleStatus = async (type, id, currentStatus) => {
    try { await infrastructureService.toggleStatus(type, id, !currentStatus); loadData(); }
    catch (error) { console.error("Error toggling status", error); }
  };

  // --- EQUIPMENT ---
  const addEquipmentRow    = () => setEquipmentList([...equipmentList, { name: '', quantity: 1, asset_id: '' }]);
          ...
          <div className="p-5 space-y-4">
            {showCampusForm && (
              <div className="bg-emerald-50/30 border border-emerald-200 rounded-xl p-5 mb-6">
                <h4 className="text-sm font-bold text-emerald-700 mb-4">
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden">
              {campuses.length === 0 && !showCampusForm && (
                <p className="p-5 text-sm text-slate-500 text-center">No campuses added yet.</p>
              )}
          ...
          <div className="p-5">
            {showRoomForm && (
              <div className="bg-blue-50/40 border border-blue-100 rounded-xl p-6 mb-6 relative transition-all">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-5 mb-5">
                  {isBulkMode ? (
                    <>
                      ...
                      {/* ✅ Room Type with "Other" support — spans extra col so text field fits */}
                      <div className="md:col-span-4">
                        <RoomTypeField
                          value={newRoom.type}
                          customValue={newRoom.customType || ''}
                          onTypeChange={handleRoomTypeChange}
                          onCustomChange={(e) => setNewRoom({...newRoom, customType: e.target.value})}
                          required
                        />
                      </div>
                    </>
                ...
                {/* ── Equipment ── */}
                <div className="border border-slate-200 bg-white rounded-lg p-5 mb-5 shadow-sm mt-3">
                  <p className="text-xs font-bold text-slate-600 mb-3">
                    Default Equipment ({equipmentList.length})
                    {isBulkMode && <span className="text-emerald-500 ml-2 italic font-medium">— Duplicated across all {newRoom.count || '0'} rooms</span>}
                  </p>
                  {equipmentList.map((eq, idx) => (
                    <div key={idx} className="flex gap-3 items-center mb-3">
                      <input type="text" placeholder="Equipment Name (e.g. Projector)" value={eq.name} onChange={(e) => updateEquipmentRow(idx, 'name', e.target.value)} className="flex-1 px-3 py-2.5 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-400 shadow-sm" />
                      ...
                      <input type="text" placeholder="Asset ID (Optional)" value={eq.asset_id} onChange={(e) => updateEquipmentRow(idx, 'asset_id', e.target.value)} className="w-40 px-3 py-2.5 border border-slate-200 rounded-lg text-sm outline-none focus:border-blue-400 shadow-sm" />
                ...
                <div className="flex justify-end gap-3 pt-2 border-t border-blue-100/50">
                  ...
                  <button
                    ...
                    disabled={savingBulk}
                    className={`px-6 py-2.5 rounded-lg text-white font-bold text-sm flex items-center gap-2 outline-none transition-all disabled:opacity-70 shadow-md ${isBulkMode ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-200' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-200'}`}
                  >
