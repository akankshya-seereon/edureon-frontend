...
// 🎯 HELPER: Safely grabs the token (Standardized)
const getAuthConfig = () => {
        ...
        <div id="salary-slip-content" className="p-10 text-slate-800 bg-white">
          ...
          <div className="grid grid-cols-2 gap-12 mb-10 border-y border-slate-100 py-8">
            <div>
              ...
              <p className="text-md font-bold text-slate-600">{record.designation || 'Employee'}</p>
              <p className="text-sm text-slate-500">{record.department || 'General Dept'}</p>
            </div>
            <div className="text-right">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center py-2">
              ...
              <span className="font-black text-slate-900">₹{Number(record.amount || 0).toLocaleString()}</span>
            </div>
            ...
            <div className="mt-8 pt-6 border-t-2 border-slate-900 flex justify-between items-end">
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-medium italic">Amount in words: Rupees {Number(record.amount || 0).toLocaleString('en-IN')} Only</p>
              </div>
              <div className="text-right">
                <p className="text-3xl font-black text-blue-700 tracking-tighter">₹{Number(record.amount || 0).toLocaleString()}</p>
              </div>
            </div>
          </div>
...
// ─── Main Component ───────────────────────────────────────────────────────────
export const Salary = () => {
  const [employees, setEmployees] = useState([]); // 🚀 Changed from faculty
  ...
  const [form, setForm] = useState({
    employeeId: "", month: "", amount: "", // 🚀 Changed from facultyId
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 🚀 FIXED: Fetch all employees instead of just faculty
      const empRes = await axios.get(`${apiBaseUrl}/admin/employees/list`, config);
      // Safely extract the array no matter how the backend formats it
      const empList = empRes.data.employees || empRes.data.data || empRes.data || [];
      const activeEmp = empList.filter(e => !e.status || e.status.toLowerCase() === "active");
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      faculty_id: form.employeeId, // Keeping database column names intact
      faculty_name: selectedEmployee?.name || `${selectedEmployee?.firstName || ''} ${selectedEmployee?.lastName || ''}`.trim() || 'Unknown', 
      designation: selectedEmployee?.designation || selectedEmployee?.role || 'Employee',
      department: selectedEmployee?.department || selectedEmployee?.dept || 'General', 
      month: form.month,
      amount: form.amount,
      method: form.method,
    ...
    try {
      ...
      setForm({ employeeId: "", month: "", amount: "", method: "Online Payment", onlineProvider: "GPay", txnNumber: "", note: "" });
    ...
    <>
      ...
      <div className="w-full font-sans text-left pb-12">
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1.5 text-md">Process and track employee salary payments dynamically</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {[
            ...
            // 🚀 FIXED: Shows count of all employees now
            { label: "Active Employees", value: employees.length, icon: Users, color: "emerald"},
          ].map((s, i) => {
            const Icon = s.icon;
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-fit">
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-6 py-5 text-left">
              <div className="flex items-center gap-3 text-left">
                ...
                <div className="text-left">
                  ...
                  <p className="text-violet-200 text-sm mt-0.5">Process employee payment via DB</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-6 space-y-4">
              <div className="space-y-1.5 text-left">
                {/* 🚀 FIXED: Label updated to Employee */}
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Select Employee *</label>
                <select className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 bg-slate-50 outline-none focus:border-blue-500" value={form.employeeId} onChange={e => setForm({...form, employeeId: e.target.value})}>
                  <option value="">Choose employee...</option>
                  {employees.map(e => <option key={e.id} value={e.id}>{e.name || `${e.firstName || ''} ${e.lastName || ''}`} — {e.designation || e.role || 'Employee'}</option>)}
                </select>
              </div>
              ...
              {selectedEmployee && (
                <div className="p-3 bg-violet-50 border border-violet-100 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-black text-md uppercase">
                    {(selectedEmployee.name || selectedEmployee.firstName)?.[0]}
                  </div>
                  <div className="text-left">
                    <p className="font-black text-slate-800 text-sm">{selectedEmployee.name || `${selectedEmployee.firstName || ''} ${selectedEmployee.lastName || ''}`.trim()}</p>
                    <p className="text-xs font-bold text-slate-500 uppercase tracking-tight">{selectedEmployee.designation || selectedEmployee.role || 'Employee'}</p>
                  </div>
                </div>
              )}
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden h-fit">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search employee, month, txn..."
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-100">
                  {/* 🚀 FIXED: Header updated to 'Employee' */}
                  <tr>{["Employee","Month","Amount","Txn ID","Actions"].map(h=>(
                    ...
                    <tr key={r.id} className="hover:bg-slate-50 transition-colors group" style={{ animation:`fadeIn 0.3s ease ${i*40}ms both` }}>
                      <td className="px-4 py-3">
                        <div>
                          ...
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">{r.department || 'General'}</p>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-sm font-bold text-slate-600">{r.month}</td>
                      <td className="px-4 py-3 text-sm font-black text-slate-900">₹{Number(r.amount || 0).toLocaleString()}</td>
      ...
      {viewRec && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm" onClick={e => e.target === e.currentTarget && setViewRec(null)}>
           <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
             ...
             <div className="p-6 space-y-4">
               {Object.entries({ Name: viewRec.faculty_name, Month: viewRec.month, Amount: `₹${Number(viewRec.amount || 0).toLocaleString()}`, Method: viewRec.method, ID: viewRec.txn_id }).map(([k,v]) => (
