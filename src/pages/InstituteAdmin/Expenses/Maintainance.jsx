    ...
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[9999]" style={{animation:"toastIn 0.3s ease"}}>
      <div className="bg-slate-900 text-white px-5 py-4 rounded-2xl shadow-2xl flex items-center gap-4 min-w-[320px]">
        ...
        <div className="flex-1">
          <p className="text-xs font-black uppercase tracking-widest text-slate-400">Maintenance Paid</p>
          <p className="font-mono font-bold text-sm mt-0.5">{txnId}</p>
        </div>
        <button onClick={copy} className={`px-3 py-1.5 rounded-xl text-xs font-black transition-all ${copied?"bg-emerald-500":"bg-white/10 hover:bg-white/20"}`}>{copied?<Check size={12}/>:<Copy size={12}/>}</button>
...
const DetailModal = ({ record, onClose }) => (
  <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
    ...
    <div className="bg-white w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden" style={{animation:"modalIn 0.25s cubic-bezier(0.16,1,0.3,1)"}}>
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 px-6 py-5 flex justify-between items-center">
        <div>
          <p className="text-blue-100 text-[10px] font-black uppercase tracking-widest">Maintenance Record</p>
          <p className="text-white font-black text-lg mt-0.5">{record.item}</p>
        </div>
...
export const Maintenance = () => {
  ...
  const [form, setForm] = useState({ item:"", month:"", reason:"", amount:"", method:"Online", txnNumber:"" });
  const reload = () => setRecords(getRecords(MAINTENANCE_KEY));
  useEffect(()=>{ reload(); },[]);
  ...
  const validate = () => {
    const e = {};
    if (!form.item.trim())   e.item   = "Enter item name";
    if (!form.month)         e.month  = "Select a month";
    if (!form.reason.trim()) e.reason = "Enter reason";
  ...
  const handleSubmit = () => {
    if (!validate()) return;
    setSaving(true);
    const txnId = form.txnNumber.trim() || genTxnId("MNT");
    setTimeout(()=>{
      addRecord(MAINTENANCE_KEY, {
        id:Date.now(), txnId, item:form.item, month:form.month,
        reason:form.reason, amount:Number(form.amount),
        method:form.method, status:"Paid",
        date:new Date().toISOString().split("T")[0],
      });
      reload(); setSaving(false);
      setToast(txnId);
      setForm({item:"",month:"",reason:"",amount:"",method:"Online",txnNumber:""});
      setErrors({});
  ...
  return (
    <>
      ...
      <div className="w-full font-sans text-left pb-12">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">Maintenance Expenses</h1>
          <p className="text-slate-500 mt-1.5 text-sm">Track repairs, upkeep and facility maintenance costs</p>
        </div>
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="bg-gradient-to-r from-blue-600 to-blue-600 px-6 py-5">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white/20 rounded-xl"><Wrench size={18} className="text-white"/></div>
                <div>
                  <h2 className="text-white font-black text-lg">Add Maintenance</h2>
                  <p className="text-blue-100 text-xs mt-0.5">Log a maintenance expense</p>
                </div>
              </div>
            </div>
            <div className="p-6 space-y-4">
              <InputField label="Item / Description" value={form.item} onChange={h("item")} error={errors.item} required placeholder="e.g. Generator repair, HVAC service" />
              <SelectField label="Month" value={form.month} onChange={h("month")} error={errors.month} required options={MONTHS} />

              <div className="space-y-1.5">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-500">Reason <span className="text-red-500">*</span></label>
                <textarea value={form.reason} onChange={h("reason")} rows={2} placeholder="Describe the reason for maintenance..."
                  className={`w-full px-3.5 py-2.5 rounded-xl border text-sm font-medium outline-none transition-all resize-none
                    ${errors.reason?"border-red-400 bg-red-50":"border-slate-200 bg-slate-50 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 focus:bg-white"}`}/>
                {errors.reason && <p className="text-xs text-red-500 flex items-center gap-1"><AlertCircle size={10}/>{errors.reason}</p>}
              </div>

              <InputField label="Amount" value={form.amount} onChange={h("amount")} error={errors.amount} required placeholder="0" prefix="₹" type="number"/>
              <SelectField label="Payment Method" value={form.method} onChange={h("method")} options={["Online","Offline/Cash","Cheque","UPI"]}/>
              <InputField label="Transaction Number (Optional)" value={form.txnNumber} onChange={h("txnNumber")} placeholder="Enter if already paid" />
          ...
          <div className="lg:col-span-3 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
              <div className="relative flex-1">
                ...
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search item, month, txn..."
              ...
              <button onClick={reload} className="p-2 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-xl"><RefreshCw size={15}/></button>
            </div>
            {filtered.length===0 ? (
              <div className="py-20 text-center">
                <Wrench size={40} className="mx-auto text-slate-200 mb-3"/>
                <p className="text-slate-400 font-bold text-sm">No maintenance records yet</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-left">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    <tr>{["Item","Month","Reason","Amount","Txn ID",""].map(h=>(
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {filtered.map((r,i)=>(
                      <tr key={r.id} className="hover:bg-slate-50 transition-colors group" style={{animation:`fadeIn 0.3s ease ${i*40}ms both`}}>
                        ...
                        <td className="px-4 py-3 text-xs font-bold text-slate-600 whitespace-nowrap">{r.month}</td>
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={()=>setViewRec(r)} className="p-1.5 text-blue-500 hover:bg-blue-50 rounded-lg"><Eye size={13}/></button>
                            <button onClick={()=>{deleteRecord(MAINTENANCE_KEY,r.id);reload();}} className="p-1.5 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={13}/></button>
                          </div>
                        </td>
                      </tr>
                    ))}
