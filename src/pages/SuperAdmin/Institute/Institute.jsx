  ...
  useEffect(() => {
    const fetchInstitute = async () => {
      try {
        ...
        if (!instituteId) {
          setError(
            `Could not find your institute. ` +
            `(User keys: [${Object.keys(user).join(", ")}]) — Please re-login.`
          );
          setLoading(false);
          return;
        }

        // Fetch all data that SuperAdmin entered when creating this institute
        ...
        if (!res.ok) {
          ...
          if (res.status === 404) throw new Error(`Institute not found (${instituteId}).`);
